...
class EyegazeClassifier(object):
    ...
    def _get_signal_props(self, data):
        if np.all(np.isnan(data['vel'])): return np.nan,np.nan,np.nan,np.nan
    ...
    def get_adaptive_saccade_velocity_velthresh(self, vels):
        ...
        def _get_thresh(cut):
            ...
            nppf34=0.6744897501960817
            scale = np.median((np.abs(vel_uthr - med))/ nppf34)
    ...
    def preproc(
        ...
        if median_filter_length:
            ...
            med_velocities = np.zeros(data['x'].size, velocities.dtype)
            med_velocities[1:] = (
